...
import { selectCustomScrollbarsEnabled } from '@olorin/core/OlorinSettings/olorinSettingsSlice';
...
/**
 * OlorinScrollbars Component for app-level scrollbar
 * Unlike normal scrollbars, this component supports mobile device as well
 */
const OlorinScrollbars = forwardRef<HTMLDivElement, OlorinScrollbarsProps>((props, ref) => {
	const {
		className = '',
		children,
		id = '',
	...
	const hookUpEvents = useCallback(() => {
		Object.keys(handlerNameByEvent).forEach((key) => {
			const callback = props[handlerNameByEvent[key] as keyof OlorinScrollbarsProps] as (
				T: HTMLDivElement,
			) => void;
...
export default OlorinScrollbars;
