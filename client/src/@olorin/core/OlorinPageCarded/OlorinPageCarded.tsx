...
import OlorinPageCardedSidebar from './OlorinPageCardedSidebar';
import OlorinPageCardedHeader from './OlorinPageCardedHeader';

const headerHeight = 120;
const toolbarHeight = 64;

/**
 * Props for the OlorinPageCarded component.
 */
type OlorinPageCardedProps = SystemStyleObject<Theme> & {
	className?: string;
...
const Root = styled('div')<OlorinPageCardedProps>(({ theme, ...props }) => ({
	display: 'flex',
	...
	'& .OlorinPageCarded-scroll-content': {
		height: '100%',
	},
	...
	'& .OlorinPageCarded-contentWrapper': {
		display: 'flex',
		flexDirection: 'column',
		flex: '1 1 auto',
		overflow: 'auto',
	...
	'& .OlorinPageCarded-sidebarHeaderInnerSidebar': {
		backgroundColor: 'transparent',
		color: 'inherit',
		height: 'auto',
		minHeight: 'auto',
	...
	'& .OlorinPageCarded-backdrop': {
		position: 'absolute',
	},
}));

/**
 * The OlorinPageCarded component is a carded page layout with left and right sidebars.
 */
const OlorinPageCarded = forwardRef<
	{ toggleLeftSidebar:(T: boolean) => void; toggleRightSidebar: (T: boolean) => void },
	OlorinPageCardedProps
>((props, ref) => {
	const {
		scroll = 'page',
		className,
	...
	return (
		<>
			...
			/>
			<Root
				className={clsx('OlorinPageCarded-root', `OlorinPageCarded-scroll-${props.scroll}`, className)}
				ref={rootRef}
				scroll={scroll}
				leftSidebarWidth={leftSidebarWidth}
				rightSidebarWidth={rightSidebarWidth}
			>
				{header && <OlorinPageCardedHeader header={header} />}

				<div className="container relative z-10 flex h-full flex-auto flex-col overflow-hidden rounded-t-16 shadow-1">
					<div className="OlorinPageCarded-wrapper">
						{leftSidebarContent && (
							<OlorinPageCardedSidebar
								position="left"
						...
						<OlorinScrollbars
							className="OlorinPageCarded-contentWrapper"
							enable={scroll === 'content'}
						>
							{content && <div className={clsx('OlorinPageCarded-content')}>{content}</div>}
						</OlorinScrollbars>
						{rightSidebarContent && (
							<OlorinPageCardedSidebar
								position="right"
...
export default memo(styled(OlorinPageCarded)``);
