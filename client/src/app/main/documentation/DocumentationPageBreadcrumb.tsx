...
const getPathTree = (departmentTree: OlorinNavItemType, url: string) => {
	function findPath(node: OlorinNavItemType, _url: string) {
		...
		if (node.children) {
			...
			for (const child of node.children) {
				const childPath = findPath(child, _url) as OlorinNavItemType[] | boolean;
	...
	const response = findPath(departmentTree, url) as OlorinNavItemType[];
...
function DocumentationPageBreadcrumb(props: DocumentationPageBreadcrumbProps) {
	...
	const pathArr: OlorinNavItemType[] = getPathTree(DocumentationNavigation, pathname);
