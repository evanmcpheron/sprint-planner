...
renderer.paragraph = (text) => `<Typography className="text-14 mb-32" component="div">${text}</Typography>\n`;

renderer.code = (code, lang) => {
	const response = `
<OlorinHighlight component="pre" className="language-${lang}">
{%% 
${code}
%%}
</OlorinHighlight>
...
function writePage(fileDir: string) {
	...
	const content = `import OlorinExample from '@olorin/core/OlorinExample';
...
async function removeUnnecessaryFiles() {
	return filewalker(demoDir)
		.then((list) => list.forEach(async (file) => {
				const extToRemove = [
					'.preview',
					'.tsx.preview',
					// '.js',
