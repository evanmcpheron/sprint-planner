...
export const scrumboardApiMocks = (mock: ExtendedMockAdapter) => {
	 ...
	 */
	mock.onGet('/scrumboard/boards').reply(() => [200, boardsDB]);
	...
	mock.onPost('/scrumboard/boards').reply(({ data }) => {
		const newBoard = { id: OlorinUtils.generateGUID(), ...JSON.parse(data as string) } as ScrumboardBoard;
	 ...
	 */
	mock.onGet('/scrumboard/members').reply(() => [200, membersDB]);
};
