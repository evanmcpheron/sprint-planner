...
export const profileApiMocks = (mock: ExtendedMockAdapter) => {
	mock.onGet('/profile/timeline').reply(() => [200, timelineApi]);

	mock.onGet('/profile/photos-videos').reply(() => [200, photosVideosApi]);

	mock.onGet('/profile/about').reply(() => [200, aboutApi]);
};
