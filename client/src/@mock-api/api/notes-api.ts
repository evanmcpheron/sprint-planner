...
export const notesApiMocks = (mock: ExtendedMockAdapter) => {
	mock.onGet('/notes').reply(() => [200, _.filter(notesDB, { archived: false })]);

	mock.onPost('/notes').reply(({ data }) => {
		const newNote = { id: OlorinUtils.generateGUID(), ...JSON.parse(data as string) } as NotesNote;
	...
	mock.onGet('/notes/archive').reply(() => [200, _.filter(notesDB, { archived: true })]);

	mock.onGet('/notes/reminders').reply(() => [200, _.filter(notesDB, (item) => item.reminder && !item.archived)]);

	mock.onGet('/notes/labels').reply(() => [200, labelsDB]);

	mock.onPost('/notes/labels').reply(({ data }) => {
		const newLabel = { id: OlorinUtils.generateGUID(), ...JSON.parse(data as string) } as NotesLabel;
