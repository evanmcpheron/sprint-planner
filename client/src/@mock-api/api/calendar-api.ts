...
export const calendarApiMocks = (mock: ExtendedMockAdapter) => {
	mock.onGet('/calendar/labels').reply(() => [200, labelsDB]);

	mock.onPost('/calendar/labels').reply(({ data }) => {
		const newLabel = { id: OlorinUtils.generateGUID(), ...JSON.parse(data as string) } as Label;
	...
	mock.onGet('/calendar/events').reply(() => [200, eventsDB]);

	mock.onPost('/calendar/events').reply(({ data }) => {
		const newEvent = { id: OlorinUtils.generateGUID(), ...JSON.parse(data as string) } as Event;
