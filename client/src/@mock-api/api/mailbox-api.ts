...
export const mailBoxApiMocks = (mock: ExtendedMockAdapter) => {
	...
	mock.onGet('/mailbox/folders').reply(() => [200, foldersDB]);

	mock.onGet('/mailbox/filters').reply(() => [200, filtersDB]);

	mock.onGet('/mailbox/labels').reply(() => [200, labelsDB]);
			...
			_.assign(
				...
				mailsDB.map((_mail) =>
					ids.includes(_mail.id)
						? {
								..._mail,
								labels: _mail.labels.includes(value as string)
									? _mail.labels
									: [..._mail.labels, value],
							}
						: _mail,
